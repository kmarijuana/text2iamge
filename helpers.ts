...
import {
  ArtifactTypeMap,
  FinishReasonMap,
} from "./api-interfaces-main/gooseai/generation/generation_pb";
...
export async function onGenerationComplete(response: GenerationResponse) {
  ...
  response.imageArtifacts.forEach((artifact: Generation.Artifact) => {
    promises.push(savefile(artifact));
  });
  let ar = await Promise.all(promises)
    .then((res) => res)
    .catch((e) => {
      console.error(e);
    });
...
function savefile(artifact: Generation.Artifact) {
  return new Promise((resolve) => {
    try {
      let image = `${define.file_dir}image-${artifact.getSeed()}.png`;
      if (!fs.existsSync(define.file_dir)) {
        fs.mkdirSync(define.file_dir);
      }
      fs.writeFileSync(image, Buffer.from(artifact.getBinary_asU8()));
      resolve(image);
    } catch (error) {
